...
const ListsPage: React.FC = () => {
  ...
  const loadUserLists = async () => {
    try {
      ...
      console.log('Listas recargadas:', lists); // <-- Log de depuración
      setUserLists(lists);
  ...
  const renderUserLists = () => {
    ...
    return (
      <div className="lists-container">
        <div className="lists-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h3 style={{ margin: 0 }}>Mis Listas ({userLists.length})</h3>
          <IonButton
            size="small"
            fill="outline"
