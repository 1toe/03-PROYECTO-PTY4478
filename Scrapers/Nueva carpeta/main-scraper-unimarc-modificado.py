...
'''
Script principal para hacer scraping de listados de productos de Unimarc según el tipo 
de sellos (advertencias nutricionales).

Funcionalidades principales:
1. Lee URLs de categorías con filtros de sellos desde un archivo configurado (urls_unimarc.txt).
2. Para cada tipo de sello y URL:
   - Recorre todas las páginas de resultados paginados
   - Extrae información básica de los productos listados (nombre, marca, SKU, precio)
   - Obtiene las URLs de detalle de cada producto para procesamiento posterior
3. Guarda los resultados en:
   - Archivos HTML de las páginas originales
   - JSON con la información extraída de productos por tipo de sello
   - Archivos de texto con las URLs de detalle para usar en otros scripts

Este script está diseñado para realizar la primera fase del proceso de scraping:
recolectar las URLs de los productos para luego procesarlas individualmente
con otros scripts más específicos.
'''
...
def scrape_products(base_url, sellos_tipo):
    all_products = []
    product_detail_urls = [] # NUEVO: Inicializar lista para URLs de detalle
    page = 1
    total_products = None
    ...
    while True:
        ...
        # NUEVO: Recolectar URLs de detalle
        for product_data in products_in_page:
            if product_data.get("url_producto"):
                product_detail_urls.append(product_data["url_producto"])
    ...
    # NUEVO: Guardar URLs de detalle de productos
    if product_detail_urls:
        categoria = base_url.split("/category/")[1].split("?")[0].replace("/", "_")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        urls_filename = f"detalle_urls_{categoria}_{sellos_tipo}_{len(product_detail_urls)}_productos_{timestamp}.txt"
        
        detail_urls_folder = "Detail URLs Unimarc"
        # os.makedirs(detail_urls_folder, exist_ok=True) # This is now handled in main()
        
        urls_path = os.path.join(detail_urls_folder, urls_filename)
        
        with open(urls_path, "w", encoding="utf-8") as f_urls:
            for url_prod in product_detail_urls:
                if url_prod:
                    f_urls.write(f"{url_prod}\n")
        print(f"\nURLs de detalle de productos guardadas para {sellos_tipo}: {len(product_detail_urls)}")
        print(f"Archivo de URLs de detalle guardado como: {urls_path}")

    # Modificado: Retorno tanto los productos como las URLs de detalle
    return all_products, product_detail_urls

def main():
    urls_folder = "URLS Unimarc"
    json_folder = "JSON Unimarc"
    detail_urls_output_folder = "Detail URLs Unimarc" # NUEVO
    os.makedirs(urls_folder, exist_ok=True)
    os.makedirs(json_folder, exist_ok=True)
    os.makedirs(detail_urls_output_folder, exist_ok=True) # NUEVO
    ...
    all_products = {}
    grand_total = 0
    # NUEVO: Lista para almacenar todas las URLs de detalle
    all_detail_urls = []
    ...
    for sello_tipo, url in urls.items():
        ...
        print(f"{'='*50}")
        # Modificado: Ahora recibimos también las URLs de detalle
        products, detail_urls = scrape_products(url, sello_tipo)
        all_products[sello_tipo] = products
        # NUEVO: Agregamos las URLs de detalle a la lista general
    ...
    # NUEVO: Guardar todas las URLs de detalle en un solo archivo consolidado
    # Eliminamos duplicados convirtiendo a conjunto y luego de vuelta a lista
    unique_detail_urls = list(set(all_detail_urls))
    consolidated_urls_path = os.path.join(detail_urls_output_folder, "urls-productos.txt")
    
    with open(consolidated_urls_path, "w", encoding="utf-8") as f_consolidated:
        for url in unique_detail_urls:
            f_consolidated.write(f"{url}\n")
    
    print(f"\nArchivo consolidado de URLs de detalle generado: {consolidated_urls_path}")
    print(f"Total de URLs únicas en archivo consolidado: {len(unique_detail_urls)}")
